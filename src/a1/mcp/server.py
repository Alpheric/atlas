...
async def embed(
    ...
    try:
        ...
        return json.dumps({"dims": len(emb), "embedding": emb[:8], "truncated": True,
...
async def list_models() -> str:
    ...
    atlas_models = ["atlas-plan", "atlas-code", "atlas-secure", "atlas-infra",
                    "atlas-data", "atlas-books", "atlas-audit", "auto"]
...
async def list_files(purpose: str = "") -> str:
    ...
    return json.dumps([
        {"id": r.id, "filename": r.filename, "purpose": r.purpose,
         "bytes": r.bytes_, "created_at": int(r.created_at.timestamp())}
        for r in rows
    ], indent=2)
...
async def list_vector_stores() -> str:
    ...
    return json.dumps([
        {"id": r.id, "name": r.name, "chunks": r.chunk_count}
        for r in rows
    ], indent=2)
