    ...
    async def _register_litellm_providers(self):
        ...
        if settings.vertex_project_id or settings.vertex_api_key:
            ...
            if settings.vertex_auth_type == "api_key" and settings.vertex_api_key:
                ...
                log.info(
                    f"Registering Vertex/Gemini via LiteLLM "
                    f"(gemini/ prefix, API key auth, {len(models)} models)"
                )
            else:
                prefix = "vertex_ai/"
                api_key = None
                log.info(
                    f"Registering Vertex via LiteLLM "
                    f"(vertex_ai/ prefix, GCP auth, {len(models)} models)"
                )
